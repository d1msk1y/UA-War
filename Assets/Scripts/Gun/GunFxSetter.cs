...
public class GunFxSetter : MonoBehaviour
{
    ...
    public Vector2 SetVFX(Vector3 aimPos)
    {
        if (gun.gunRaycast.collider == null)
            return gun.firePos.position + (aimPos - transform.position) * 30;
        else
            return gun.gunRaycast.point;
    }
